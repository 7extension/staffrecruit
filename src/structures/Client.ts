...
export default class InfernoClient extends Client {
  ...
  public async init() {
    ...
    this.on('modalSubmit', (modal) => {
      ...
      let result: string = `**Dlaczego akurat ty?:** ${answer1}\n**Jak masz na imie?:** ${answer2}\n**Co byś wprowadził do naszego serwera?:** ${answer3}\n**Ile masz lat?:** ${answer4}\n **Jakie masz doświadczenie?:** ${answer5}`
      console.log(format(new Date(), 'YYYY/MM/DD HH:mm') + result.replaceAll('**', ' '))
      console.log('-----------------------------------------------------')
      if (result.length > 2040) {
        result =
          'Aplikacja jest za długa zobacz konsole.'
        console.log('^^^^ Nie można wysłać wiadomości, wyslano w konsoli ^^^^')
        console.log('-----------------------------------------------------')
      }
      ...
      const embed = new MessageEmbed()
        ...
        .setTimestamp()
        .setFooter({ text: 'Prosy Największe' })
        .setColor('RANDOM')
